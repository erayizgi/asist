...
class ForecastController extends Controller
{
 ...
 order by mac_id desc limit 25 ')->result_array();
	...
	{
		try {
			$count = Forecast::select('tb_iddaa_tahminleri.*', 'tb_kullanicilar.kullaniciAdi', 'tb_kullanicilar.IMG', 'tb_kullanicilar.adSoyad')->join("tb_kullanicilar", "tb_kullanicilar.ID", "tb_iddaa_tahminleri.tahminci_id")
				->where(['mac_id' => $id, 'tahmin_durumu' => 1])
			...
			$data = DB::table('events')
				...
				->first();
			$comments = $query["query"]->select('tb_iddaa_tahminleri.*', 'tb_kullanicilar.kullaniciAdi', 'tb_kullanicilar.IMG', 'tb_kullanicilar.adSoyad')->join("tb_kullanicilar", "tb_kullanicilar.ID", "tb_iddaa_tahminleri.tahminci_id")
				->where(['mac_id' => $id, 'tahmin_durumu' => 1])
